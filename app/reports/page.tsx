...
export default async function ReportsPage() {
  ...
  const [{ data: daily, error: dailyError }, { data: maintenance, error: maintenanceError }, { data: employees }, { data: reviews }, { data: profiles }] = await Promise.all([
    supabase.from("daily_reports").select("id, employee_id, report_date, start_time, end_time, work_summary, issues, actions_taken, notes, status, submitted_at").order("report_date", { ascending: false }),
  ...
  const dailyItems: ReviewReportItem[] = (daily ?? []).map((report) => ({
    ...
    meta: [report.start_time && report.end_time ? `${report.start_time.slice(0, 5)} تا ${report.end_time.slice(0, 5)}` : null, report.issues ? `مشکلات: ${report.issues}` : null, report.actions_taken ? `اقدامات: ${report.actions_taken}` : null, report.notes].filter((value): value is string => Boolean(value)),
