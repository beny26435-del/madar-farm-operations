...
export default async function TechnicianConfirmationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: job } = await admin.from("technician_jobs").select("technician_name, item_name, customer_name, quantity, promised_return_at").eq("id", confirmation.job_id).maybeSingle();
