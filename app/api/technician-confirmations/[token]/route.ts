...
export async function POST(request: Request, context: { params: Promise<{ token: string }> }) {
  ...
  if (!parsed.success) return NextResponse.json({ message: "تاریخ تحویل معتبر نیست." }, { status: 400 });
  ...
  if (result.result === "invalid_promised_return") return NextResponse.json({ message: "تاریخ تحویل به مجموعه را مشخص کنید." }, { status: 400 });
