...
export async function POST(_request: Request, context: { params: Promise<{ token: string }> }) {
  ...
  if (result.result === "confirmed" && result.confirmation_type && result.repair_item_id) {
    await recordActivity({ actorId: null, action: `customer_confirmation.${result.confirmation_type}`, entityType: "repair_item", entityId: result.repair_item_id, metadata: { customer_name: result.customer_name ?? "مشتری", item_name: result.repair_item_name ?? "" } });
  }
