...
export default async function ExpensesPage() {
  ...
  const { data: expenses, error } = employeeIds.length
    ? await admin.from("employee_expenses").select("id, employee_id, expense_date, description, amount, invoice_path, created_at").in("employee_id", employeeIds).order("expense_date", { ascending: false }).order("created_at", { ascending: false })
    : { data: [], error: null };
  ...
  const records: ExpenseRecord[] = (expenses ?? []).map((expense) => ({ id: expense.id, employeeId: expense.employee_id, employeeName: names.get(expense.employee_id) ?? "کارمند", avatarUrl: avatarUrls.get(expense.employee_id) ?? null, expenseDate: expense.expense_date, description: expense.description, amount: expense.amount, invoiceUrl: expense.invoice_path ? invoiceUrls.get(expense.invoice_path) ?? null : null, createdAt: expense.created_at }));
