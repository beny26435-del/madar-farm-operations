...
export default async function DashboardPage() {
  ...
  const [{ data: reports }, { data: employees }, { data: tasks }] = await Promise.all([
    ...
    supabase.from("daily_tasks").select("id, title, task_date, created_by, completed_by, completed_at, created_at").is("completed_at", null).order("task_date", { ascending: false }).order("created_at", { ascending: false }).limit(50),
  ]);
