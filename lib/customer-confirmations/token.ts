...
export async function issueCustomerConfirmation(input: { itemId: string; type: CustomerConfirmationType; createdBy: string; origin: string }) {
  const admin = createAdminClient();
  const { data: existing, error: lookupError } = await admin.from("customer_handover_confirmations").select("id, confirmed_at").eq("item_id", input.itemId).eq("type", input.type).maybeSingle();
