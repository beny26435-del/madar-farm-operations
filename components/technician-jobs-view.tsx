...
export function TechnicianJobsView({ initialJobs, devices: initialDevices, loadError }: { initialJobs: TechnicianJob[]; devices: DeviceOption[]; loadError: boolean }) {
  ...
  const [reworkJob, setReworkJob] = useState<TechnicianJob | null>(null);
  ...
  async function createLink(job: TechnicianJob, type: "handover" | "return" | "rework", selectedQuantity?: number) {
    setLinkingId(job.id);
    setFormError(null);
    try {
      const response = await fetch(`/api/technician-jobs/${job.id}/confirmation`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, quantity: selectedQuantity }) });
      const result = await response.json().catch(() => ({ message: "پاسخ سرور معتبر نیست." })) as { confirmationUrl?: string; job?: TechnicianJob; sourceJobId?: string; remainingQuantity?: number; message?: string };
      ...
      if (type === "rework" && result.job) {
        setJobs((current) => result.job!.id === job.id
          ? current.map((item) => item.id === job.id ? result.job! : item)
          : [result.job!, ...current.map((item) => item.id === result.sourceJobId ? { ...item, quantity: result.remainingQuantity ?? item.quantity } : item)]);
        setReworkJob(null);
        setReworkQuantity("");
      }
      ...
      setCopied(false); setLinkResult({ links: [{ url: result.confirmationUrl, label: `${job.item_name} · ${job.customer_name}${type === "rework" ? ` · ${selectedQuantity?.toLocaleString("fa-IR")} عدد` : ""}` }], title });
  ...
  <Dialog open={Boolean(reworkJob)} onClose={() => !linkingId && setReworkJob(null)} title="تعداد دستگاه مرجوعی" description="فقط تعداد دستگاه‌های خراب را برای مرجوعی مشخص کنید." mark={<RotateCcw />}><form className="technician-create-form" onSubmit={(event) => { event.preventDefault(); if (!reworkJob) return; const selected = Number(reworkQuantity); if (!Number.isInteger(selected) || selected < 1 || selected > reworkJob.quantity) { setFormError("تعداد مرجوعی را درست وارد کنید."); return; } void createLink(reworkJob, "rework", selected); }}><div className="technician-device"><Package /><div><small>تعداد موجود در این ارجاع</small><strong>{reworkJob?.item_name}</strong></div><em>{reworkJob?.quantity.toLocaleString("fa-IR")} عدد</em></div><label className="field"><span className="field-label">تعداد مرجوعی</span><input className="input" type="number" inputMode="numeric" min="1" max={reworkJob?.quantity ?? 1} autoComplete="off" value={reworkQuantity} onChange={(event) => setReworkQuantity(event.target.value)} required /></label>{formError && <p className="technician-form-error" role="alert">{formError}</p>}<div className="dialog-actions"><button type="button" className="button button-ghost" onClick={() => setReworkJob(null)} disabled={Boolean(linkingId)}>انصراف</button><button className="button button-primary" disabled={Boolean(linkingId)}>{linkingId ? <LoaderCircle className="spinning" /> : <RotateCcw />}{linkingId ? "در حال ساخت..." : "ساخت لینک مرجوعی"}</button></div></form></Dialog>
