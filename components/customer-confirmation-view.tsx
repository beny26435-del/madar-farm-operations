...
type Confirmation = { type: "intake" | "delivery"; expiresAt: string; confirmedAt: string | null; requestedAt: string; expired: boolean; customerName: string; itemName: string; details: string | null; receivedAt: string; deliveredAt: string | null };
...
export function CustomerConfirmationView({ token, confirmation }: { token: string; confirmation: Confirmation | null }) {
  ...
  return <main className="confirmation-public-page"><section className="confirmation-public-card">
    ...
    {confirmed ? <div className="confirmation-success"><span><CheckCircle2 /></span><h1>تأیید شما ثبت شد</h1><p>{isDelivery ? "تحویل گرفتن وسیله از تعمیرگاه تأیید شد." : "تحویل دادن وسیله به تعمیرگاه تأیید شد."}</p>{confirmedAt && <time>{formatDate(confirmedAt)}</time>}</div> : <><div className="confirmation-copy"><small>{confirmation.customerName}</small><h1>{isDelivery ? "آیا این وسیله را تحویل گرفته‌اید؟" : "آیا این وسیله را برای تعمیر تحویل داده‌اید؟"}</h1><p>مشخصات زیر را بررسی کنید و فقط در صورت درست بودن، تأیید را بزنید.</p></div><article className="confirmation-item"><span>{isDelivery ? <PackageCheck /> : <Wrench />}</span><div><small>وسیله یا قطعه</small><strong>{confirmation.itemName}</strong>{confirmation.details && <p>{confirmation.details}</p>}</div></article><div className="confirmation-meta"><span>{isDelivery ? "زمان ثبت درخواست تحویل" : "زمان ثبت دریافت"}</span><strong>{formatDate(isDelivery ? confirmation.requestedAt : confirmation.receivedAt)}</strong></div>{error && <p className="confirmation-error" role="alert">{error}</p>}<button className="button button-primary confirmation-submit" onClick={confirm} disabled={pending}><CheckCircle2 />{pending ? "در حال ثبت..." : isDelivery ? "تأیید می‌کنم تحویل گرفتم" : "تأیید می‌کنم تحویل دادم"}</button><p className="confirmation-note"><ShieldCheck /> این لینک مخصوص همین وسیله است و پس از تأیید دوباره استفاده نمی‌شود.</p></>}
  </section></main>;
}
