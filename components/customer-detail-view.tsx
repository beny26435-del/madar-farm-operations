...
type RepairItem = { id: string; customer_id: string; intake_id: string | null; item_name: string; quantity: number; status: "received" | "delivered"; received_at: string; delivered_at: string | null };
...
export function CustomerDetailView({ customer, items, confirmations, loadError }: { customer: Customer; items: RepairItem[]; confirmations: Confirmation[]; loadError: boolean }) {
  ...
  async function addItem(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(`/api/customers/${customer.id}/items`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ itemName, quantity: normalizedQuantity }) });
  ...
  return <div className="app-page customer-detail-page"><div className="page-container">
    ...
    <div className="customer-detail-layout"><section className="surface repair-intake-card"><header><span><Plus /></span><div><h2>ثبت وسیله یا قطعه</h2><p>نام و تعداد مورد تحویل‌گرفته‌شده را ثبت کنید.</p></div></header><form onSubmit={addItem}><label className="field"><span className="field-label">نام وسیله یا قطعه</span><input className="input" autoComplete="off" value={itemName} onChange={(event) => { setItemName(event.target.value); setError(null); }} /></label><label className="field"><span className="field-label">تعداد</span><input className="input" inputMode="numeric" autoComplete="off" dir="ltr" value={quantity} onChange={(event) => { setQuantity(event.target.value.replace(/[^0-9۰-۹]/g, "")); setError(null); }} /></label>{error && <p className="report-form-error" role="alert">{error}</p>}<button className="button button-primary" disabled={pending}><Plus /> {pending ? "در حال ثبت..." : "ثبت برای تعمیر"}</button></form></section>
