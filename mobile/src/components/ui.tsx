...
import { ActivityIndicator, Pressable, RefreshControl, ScrollView, StyleSheet, Text, TextInput, View, type StyleProp, type ViewStyle } from "react-native";
...
export function Screen({ children, refreshing, onRefresh, contentStyle }: { children: ReactNode; refreshing?: boolean; onRefresh?: () => void; contentStyle?: StyleProp<ViewStyle> }) {
  return <SafeAreaView edges={["top"]} style={styles.safe}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[styles.screen, contentStyle]} refreshControl={onRefresh ? <RefreshControl refreshing={Boolean(refreshing)} onRefresh={onRefresh} tintColor={colors.primary} /> : undefined}>{children}</ScrollView></SafeAreaView>;
}
...
export function Button({ title, onPress, icon, tone = "primary", disabled, loading, style }: { title: string; onPress: () => void; icon?: ComponentProps<typeof Ionicons>["name"]; tone?: "primary" | "secondary" | "danger"; disabled?: boolean; loading?: boolean; style?: StyleProp<ViewStyle> }) {
  return <Pressable accessibilityRole="button" disabled={disabled || loading} onPress={onPress} style={({ pressed }) => [styles.button, styles[`button_${tone}`], (disabled || loading) && styles.disabled, pressed && styles.pressed, style]}>{loading ? <ActivityIndicator color={tone === "secondary" ? colors.primary : "#fff"} /> : <>{icon ? <Ionicons name={icon} size={18} color={tone === "secondary" ? colors.primary : "#fff"} /> : null}<Text style={[styles.buttonText, tone === "secondary" && styles.buttonTextSecondary]}>{title}</Text></>}</Pressable>;
}
...
export function ErrorBanner({ message }: { message: string }) { return <View style={styles.errorBanner}><Ionicons name="alert-circle-outline" size={20} color={colors.danger} /><Text>{message}</Text></View>; }
export function SuccessBanner({ message }: { message: string }) { return <View style={styles.successBanner}><Ionicons name="checkmark-circle-outline" size={20} color={colors.success} /><Text>{message}</Text></View>; }
...
const styles = StyleSheet.create({
  ...
  screen: { padding: 18, paddingBottom: 120, gap: 16 },
  ...
  card: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: 22, padding: 17, gap: 13, shadowColor: "#15231B", shadowOffset: { width: 0, height: 7 }, shadowOpacity: 0.05, shadowRadius: 18, elevation: 2 },
  button: { minHeight: 50, borderRadius: 15, paddingHorizontal: 18, flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 8 },
  ...
  input: { minHeight: 52, borderRadius: 15, borderWidth: 1, borderColor: colors.border, backgroundColor: "#FCFDFC", paddingHorizontal: 15, color: colors.text, fontSize: 15, writingDirection: "rtl" },
  ...
  errorBanner: { borderRadius: 14, backgroundColor: colors.dangerSoft, padding: 13, flexDirection: "row-reverse", gap: 8, alignItems: "center" }, successBanner: { borderRadius: 14, backgroundColor: colors.successSoft, padding: 13, flexDirection: "row-reverse", gap: 8, alignItems: "center" },
